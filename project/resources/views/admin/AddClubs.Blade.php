...
<div class="container mt-5">
    <div class="card shadow-lg">
        ...
        <div class="card-body">
            ...
            {{--  Add Club Form --}}
            <form action="{{ route('Clubs.edit') }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('tilte') is-invalid @enderror" 
                           id="tilte" name="tilte" placeholder="Enter club name" value="{{ old('tilte') }}" required>
                    @error('tilte')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" 
                              id="description" name="description" rows="4" placeholder="Enter club description" required>{{ old('description') }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('date_creation') is-invalid @enderror" 
                           id="date_creation" name="date_creation" value="{{ old('date_creation') }}" required>
                    @error('date_creation')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
